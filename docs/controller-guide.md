...
它的特征很典型——**外形是 PS 手柄，按键印刷却是 Xbox 式**（A 在下、B 在右、X 在左、Y 在上），而且**只有 SELECT / START / MODE，没有 Guide（中央）键**。市面上不少手柄都是这个样子。
...
| 情况 | 表现 |
|---|---|
| **内置名册里没有的手柄** | 会自动套用 **Xbox 式**映射（A 在下）、热键取 **SELECT**，多数情况直接可用 |
| **内置名册里的其他手柄** | 键位可用；但热键**未逐一适配**，若遇到退不出模拟器之类的问题，跑一次设定精灵即可 |
| **任天堂式手柄**（A 在右侧） | 默认值不合用，请跑一次设定精灵 |
