...
> **⚠️ 现况（2026-07，独立 PPSSPP 尚未接透传）**：上面「按印刷」是**目标设计**。独立 PPSSPPSDL 的热键目前
> **写死 Xbox 预设**——`controls.ini` 的 `SELECT+X`（呼出菜单）和弦码为 `10-196:10-191`（X 在西）。
> 对 **Xbox 式手柄正确**；**任天堂式**（X 印在北=`10-188`）会落到错的物理键（西/印着 Y 那颗）。
> 原因：①静态模板写死；②`set_ppsspp_joy.sh` 的 `EE_INVERT_*` 只管第三层面键位置对齐、**不读布局侦测**；
> ③本机未见 ES 把布局结果暴露成可读设定。要真正「按印刷+随布局」需**接透传**：
> ES4All 把第一层 `InvertButtons` 暴露成 `get_ee_setting` → joy 脚本据此写 `10-191`(Xbox)/`10-188`(任天堂)。
